...
void MapDrawer::DrawPath(Node* goal) {
	...
	while (currentNode != NULL) {
		...
		if (nextNode == NULL) {
			MapDrawer::SetPointType(currentNode->getY(), currentNode->getX(),
					PathStart);
...
double MapDrawer::DrawPatricles(
		std::vector<LocalizationParticle *>* particles) {
	...
	for (unsigned i = 0; i != particles->size(); i++) {
		...
		if (i >= particles->size() - 30) {
			MapDrawer::SetPointType(particale->row, particale->col,
					GoodParticle);
